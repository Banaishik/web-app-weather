...
import { ListFavoriteCity } from "./components/favoriteBlock/ListFavorite.tsx";
import { ContainerBlock } from "./components/containerBlock/containerBlock.tsx";
...
const App : FC = () => {
  ...
  const addCityToFavorite = async (city : string) : Promise<void> => {
    try {
      ...
      if (state.favoriteCity.indexOf(city) === -1) {  
        ...
        updatedFavoriteCity = state.favoriteCity.filter(item => item !== city)
      }
  ...
  const deleteFavoriteCity = (city : string) : void => {
    const newArray : string[] = state.favoriteCity.filter(item => item!== city)
